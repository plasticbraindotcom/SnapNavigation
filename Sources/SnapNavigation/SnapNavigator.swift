...
extension SnapNavigator {
    ...
    public var navigationProvider: SnapNavigatorDataSource? {
        get { return nil }
        set { }
    }
    
    // Designated navigate function.
    public func navigate(using navigation: SnapNavigation) {
    ...
    public func navigate(from source: SnapNavigation.Source, to destination: SnapNavigation.Destination, applying mediation: SnapNavigation.Mediation?, with presentation: SnapNavigation.Presentation?) {
        let nav = SnapNavigation(source: source, destination: destination, mediation: mediation, presentation: presentation)
        navigate(using: nav)
    }
    
    public func navigate(from source: SnapNavigation.Source, to destination: SnapNavigation.Destination, with intent: SnapNavigationIntent) {
        let nav = SnapNavigation(source: source, destination: destination, mediation: intent.mediation, presentation: intent.presentation)
        navigate(using: nav)
    }
    
    public func navigate(from source: SnapNavigation.Source, to destination: SnapNavigation.Destination, with presentation: SnapNavigation.Presentation) {
    ...
    public func navigate(with intent: SnapNavigationIntent) {
        guard let nav = navigationProvider?.navigation(for: self) else { return }
        navigate(from: nav.source, to: nav.destination, with: intent)
    }
    
    public func navigate(applying mediation: SnapNavigation.Mediation) {
        guard let nav = navigationProvider?.navigation(for: self) else { return }
        navigate(from: nav.source, to: nav.destination, applying: mediation, with: nav.presentation)
    }
    
    public func navigate(applying mediationMethod: @escaping (UIViewController, UIViewController) -> ()) {
        guard let nav = navigationProvider?.navigation(for: self) else { return }
        navigate(from: nav.source, to: nav.destination, applying: .method(mediationMethod), with: nav.presentation)
    }
    
    public func navigate(with presentation: SnapNavigation.Presentation) {
        guard let nav = navigationProvider?.navigation(for: self) else { return }
        navigate(from: nav.source, to: nav.destination, applying: nav.mediation, with: presentation)
    }
    
    public func navigate(with presentationMethod: @escaping (UIViewController, UIViewController) -> ()) {
        guard let nav = navigationProvider?.navigation(for: self) else { return }
    ...
    public func destinationViewController(of navigation: SnapNavigation) -> UIViewController {
    ...
    public func sourceViewController(of navigation: SnapNavigation) -> UIViewController {
